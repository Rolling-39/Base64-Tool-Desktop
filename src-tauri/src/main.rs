...
// ── Stream-encode a file → output file (memory-efficient, for any size) ──
#[tauri::command]
fn encode_file_stream(input_path: String, output_path: String) -> Result<u64, String> {
    let input =
        BufReader::new(fs::File::open(&input_path).map_err(|e| format!("无法打开: {e}"))?);
    let output =
        BufWriter::new(fs::File::create(&output_path).map_err(|e| format!("无法创建: {e}"))?);

    let mut encoder =
        base64::write::EncoderWriter::new(output, &STANDARD);
    let mut reader = BufReader::new(input);
    std::io::copy(&mut reader, &mut encoder)
        .map_err(|e| format!("编码失败: {e}"))?;
    encoder.finish().map_err(|e| format!("完成失败: {e}"))?;
    // The output file is now complete; return its actual size
...
// ── Stream-decode Base64 from file → binary output file ──
#[tauri::command]
fn decode_file_stream(input_path: String, output_path: String) -> Result<u64, String> {
    let input =
        BufReader::new(fs::File::open(&input_path).map_err(|e| format!("无法打开: {e}"))?);
    let output =
        BufWriter::new(fs::File::create(&output_path).map_err(|e| format!("无法创建: {e}"))?);
    ...
    let mut decoder =
        base64::read::DecoderReader::new(input, &STANDARD);
